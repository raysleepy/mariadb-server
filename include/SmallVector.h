...
//
// This is a reduced version of the file from LLVM 15.0.4, meant only for
// storing POD elements.
//
...
protected:
  ...
  bool isSafeToReferenceAfterResize(const void *Elt, size_t NewSize) {
    // Past the end.
    if (likely(!isReferenceToStorage(Elt)))
      return true;
  ...
  static const T *reserveForParamAndGetAddressImpl(U *This, const T &Elt,
    ...
    if (likely(NewSize <= This->capacity()))
      return &Elt;

    bool ReferencesStorage = false;
    int64_t Index = -1;
    if (!U::TakesParamByValue) {
      if (unlikely(This->isReferenceToStorage(&Elt))) {
